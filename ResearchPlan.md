...
- **Learning rubrics.** Pairwise comparison of anonymized answers (usernames and any other identifying tokens are stripped before each comparison is sent to a judge). Adaptive Comparative Judgment (ACJ) picks informative pairs — stable ranking in ~10–15×N comparisons rather than full pairwise — and Bradley-Terry aggregates the pairwise wins into interval scores.
  
  *Judge pool and prompt variation.* 4–6 LLM judges drawn from different training lineages (e.g., Anthropic, OpenAI, Google, Mistral, plus at least one model trained on a substantively different data mix such as DeepSeek or Qwen), accessed via OpenRouter so judges can be added or swapped by changing a model identifier. Each judge runs with 2–3 prompt phrasings of the same comparison instruction, giving ~12–18 effective judge configurations. Each ACJ-selected pair is routed to one randomly-chosen configuration; over the full set of comparisons each configuration contributes roughly equally. Model versions are pinned (e.g., `anthropic/claude-sonnet-4.6`) and recorded with each comparison in the analysis output for reproducibility.
  
  *Independence diagnostics and residual limit.* Standard Bradley-Terry assumes pairwise judgments are independent given the latent answer qualities. With LLM judges that's an overstatement — models trained on overlapping data share specific biases (verbose-answer preferences, common shibboleths for "thoughtful explanation") that survive prompt variation. We mitigate by (i) reporting inter-judge agreement — Cohen's kappa on overlapping comparisons, or the correlation of per-judge BT rankings fit separately — as a precision diagnostic, since judges agreeing more than independence predicts signals that pooled BT standard errors are too small; and (ii) fitting a hierarchical Bradley-Terry with a per-judge-configuration random effect, which absorbs systematic per-configuration bias. The residual concern — bias shared across the entire judge pool — looks identical to "the truth" inside the model and isn't fully recoverable; we report it as a limitation rather than try to model it away.
...
- **Production:** `sum score (0–15) ~ chatbot + position + task + half + (1|student)`
- **Learning:** `z-scored BT score ~ chatbot + position + task + half + (1|student) + (1|question)`
- **Instructor interaction:** `student-initiated interaction count ~ chatbot + position + task + half + (1|student)` with a Poisson or negative binomial likelihood. This model is contingent on per-student observation granularity; if we end up with per-half or per-slot counts, the third track collapses to a descriptive comparison rather than a fitted model.

*Note on Bayesian framing.* The choice of Bayesian over frequentist mixed models here is pragmatic, not driven by informative priors. There aren't any subject-matter priors available for this study; the priors used will be weakly informative defaults (e.g., `normal(0, 1)` on standardized coefficients, half-normal on variance components) chosen for sensibility, not for incorporating prior knowledge. What the Bayesian framing actually buys: **(1)** small-sample regularization that helps mixed models with N=24 students and several variance components converge to sensible estimates — a frequentist mixed-model fit at this N often reports singular variance components or fails to converge; **(2)** posterior samples that make derived contrasts — especially the production-vs-learning dissociation across two separate models — a one-line operation on existing samples rather than a bootstrap loop or a joint-model exercise. Frequentist mixed models with student-level bootstrap CIs would be a defensible alternative; the Bayesian framing is the pragmatic choice for the convergence and contrast-arithmetic reasons, not a philosophical one.

*Note on likelihoods and predictor coding.* Each of the three models uses a likelihood matched to the shape of its outcome:

- **Production:** *beta-binomial*, treating the 0–15 sum score as 15 Bernoulli trials whose success probability depends on the linear predictor, plus an over-dispersion parameter. Beta-binomial respects the 0–15 bounds and stays integer-valued — important because we expect ceiling effects on at least some tasks (a chatbot-aided student writing a near-complete program may saturate the rubric). A Gaussian likelihood at the ceiling clips the high tail and biases the chatbot coefficient downward.
- **Learning:** *Gaussian*. The z-scored Bradley-Terry score is continuous and roughly Normal, so Gaussian residuals are appropriate. (A future refinement could carry the BT score's own posterior uncertainty into this model rather than using point estimates as outcomes; at this N the refinement is unlikely to matter materially.)
- **Instructor interaction:** *negative binomial* rather than Poisson for the count outcome — allows over-dispersion past the Poisson mean = variance constraint, which counts of student–instructor interactions almost certainly violate.

Predictor coding: continuous predictors are standardized (mean 0, SD 1) so the `normal(0, 1)` prior on coefficients is on a sensible scale; categorical predictors are sum-coded so each coefficient is a deviation from the grand mean rather than from an arbitrary baseline category.
...
*Note on `half`.* The class is split into Half A and Half B (~12 students each) with different (slot, task) mappings, which creates within-half similarity that the student random intercepts don't capture (same room corner, possibly same nearby instructor, friend-group effects, etc.). Including `half` as a fixed-effect indicator absorbs the mean shift between halves so it doesn't leak into other coefficients. We do *not* use `(1|half)` for a half-level random effect because n=2 clusters doesn't admit a variance estimate — software either fails to converge, fixes the variance at zero, or reports something arbitrary. Note that the pooled chatbot main effect is robust to between-half differences by Latin-square construction; task-specific chatbot contrasts are not, since (half, task, chatbot) cells are not balanced on their own.

*Note on power.* We do not perform a formal a priori power analysis. The chatbot's effect on production and learning is the very thing this study is trying to measure, and we have no prior data to anchor a smallest-effect-of-interest. Our stance is matched to that: if the chatbot's effect is large enough to matter pedagogically, it should be visible across 48 chatbot-on and 48 chatbot-off measurements (24 students × 2 slots of each condition) on each of the production and learning outcomes. If it isn't visible at that resolution, we don't try to push a credible interval over a significance line — "no clear effect at this scale of intervention" is itself an informative answer to the question we asked, not a failed measurement.

*Note on missing data and exclusion.* Realistically, students will miss slots — illness, schedule conflicts, technology hiccups — and the Latin-square balance assumed in the design will only approximately hold in the data. The plan:

- **Headline analysis includes every student with at least one observed slot.** Mixed-effects models handle unbalanced data natively: a student who completes 3 of 4 slots contributes those three observations to the relevant fixed effects, and their per-student random intercept is estimated from fewer points with commensurately more uncertainty. Listwise dropping would *also* discard the complete slots of incomplete students — the opposite of what we want.
- **Soft minimum: ≥ 2 observed slots per student** to enter the headline analysis. Below 2 there's barely a within-student contrast for the model to learn from; the threshold is cheap insurance and easy to lift if it ever excludes a substantively-interesting student.
- **Report a missingness table up front,** before model results — N students per (slot × half × chatbot) cell — so the reader sees the actual balance the analysis ran on rather than the planned balance.
- **Sensitivity analysis: complete-case re-fit** of the headline models on only students who completed all four slots. If chatbot estimates move materially, that's a flag that incomplete students were systematically different from complete ones — a missing-not-at-random concern (struggling students dropping Day 2 because they're frustrated; high-performers leaving early because they finished) that mixed-effects models can't automatically correct for. The headline analysis assumes missingness is random conditional on the model's predictors; the sensitivity re-fit is the main safeguard against that assumption being wrong.
